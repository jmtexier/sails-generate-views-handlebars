...
A `views-handlebars` generator for use with the Sails command-line interface.
...
sailsgen(require('sails-generate-views-handlebars'), scope, function (err) {
	if (err) throw err;
...
```sh
$ git clone YOUR_FORK_OF_THIS_REPO sails-generate-views-handlebars-fork
$ cd sails-generate-views-handlebars-fork
$ npm install
$ node ./bin
```
